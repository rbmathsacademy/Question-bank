...
import { Plus, Search, Check, X, FileText, Send, User, Download } from 'lucide-react';
...
export default function SchoolPerformancePanel({ batch, analyticsData }: { batch: string, analyticsData?: any }) {
    ...
    const handleDownloadPDF = (student: any, exam: any) => {
        if (!analyticsData || !analyticsData.analytics) {
            toast.error('Analytics data not fully loaded yet');
            return;
        }
        
        const studentStats = analyticsData.analytics.find((a: any) => a.student.phoneNumber === student.phoneNumber);
        if (!studentStats) {
            toast.error('Could not find detailed stats for this student');
            return;
        }
                            ...
                            <div className="p-4 flex flex-col gap-3">
                                {student.exams.length === 0 ? (
                                    ...
                                    student.exams.map((exam: any) => (
                                        <div key={exam._id} className="bg-slate-900/50 border border-slate-700 rounded-lg p-3 text-sm relative group">
                                            ...
                                            <div className="flex justify-between items-end mb-1 pr-12">
